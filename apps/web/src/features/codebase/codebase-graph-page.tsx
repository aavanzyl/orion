...
import { FolderIcon, Loader2Icon, GitBranchIcon } from 'lucide-react';
import type { CallGraph, DirSummary } from '@orion/models';
import {
  Select,
  SelectContent,
  SelectItem,
...
const LEGEND_ITEMS = [
  { color: 'border-emerald-400 bg-emerald-500/10', label: 'Endpoint handler' },
  { color: 'border-blue-400 bg-blue-500/10', label: 'Function' },
];

function graphToFlow(graph: CallGraph): { nodes: Node[]; edges: Edge[] } {
  return {
            ...
            <div className="flex flex-col items-center gap-2 text-center">
              {loading ? (
                <>
                  ...
                  <p className="text-sm text-muted-foreground">Analyzing call graph...</p>
                </>
              ) : (
                <>
                  <GitBranchIcon className="size-8 text-muted-foreground" />
                  <p className="text-sm text-muted-foreground">
                    No functions found matching the current filters.
                  </p>
                </>
              )}
            </div>
        ...
        {projectId ? (
          ...
          <div className="flex h-full items-center justify-center">
            <div className="flex flex-col items-center gap-2 text-center">
              <GitBranchIcon className="size-8 text-muted-foreground" />
              <p className="text-sm text-muted-foreground">
                Select a project to view its call graph.
              </p>
            </div>
          </div>
        )}

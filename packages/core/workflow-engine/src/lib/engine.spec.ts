...
describe('WorkflowEngine', () => {
  ...
  it('fans a matrix node out over a literal list, invoking the executor once per item', async () => {
    const cfg = config([
      {
        id: 'fanout',
        type: 'shell',
    ...
    const executor: NodeExecutor = {
      type: 'shell',
      execute: async (ctx) => {
        seenItems.push(ctx.matrix?.item);
        total = ctx.matrix?.total ?? total;
        return { status: 'completed', output: { stdout: `ran ${String(ctx.matrix?.item)}` } };
      },
    };
    const matrixTotals: number[] = [];
    const engine = new WorkflowEngine({
      store,
      emit: async (e) => {
        if (e.type === 'node.matrix') matrixTotals.push((e.payload as { total: number }).total);
      },
      moveTicket: async () => undefined,
      executors: [executor],
    });
    ...
    expect(total).toBe(3);
    expect(matrixTotals).toEqual([3]);
    const node = store.nodes.find((n) => n.nodeKey === 'fanout');
    expect((node?.output as { items: unknown[] }).items).toHaveLength(3);
  });

  it('resolves matrix items from an upstream node output array', async () => {
    const cfg = config([
      { id: 'plan', type: 'shell', script: 'x', swimlane: 'doing' },
      {
        id: 'fanout',
        type: 'shell',
        script: 'x',
    ...
    const executor: NodeExecutor = {
      type: 'shell',
      execute: async (ctx) => {
        if (ctx.node.nodeKey === 'plan') {
          return { status: 'completed', output: { files: ['one.ts', 'two.ts'] } };
        }
        seenItems.push(ctx.matrix?.item);
        return { status: 'completed', output: { stdout: String(ctx.matrix?.item) } };
      },
    };
    const engine = new WorkflowEngine({
      store,
    ...
    expect(run.status).toBe('completed');
    expect(seenItems.sort()).toEqual(['one.ts', 'two.ts']);
    const node = store.nodes.find((n) => n.nodeKey === 'fanout');
    expect((node?.output as { items: unknown[] }).items).toHaveLength(2);
  });

  it('fails the node when a matrix reference does not resolve to an array', async () => {
    const cfg = config([
      { id: 'plan', type: 'shell', script: 'x', swimlane: 'doing' },
      {
        id: 'fanout',
        ...
        dependsOn: ['plan'],
        swimlane: 'doing',
        matrix: { items: 'nodes.plan.finalResponse' },
      },
    ]);
    const executor: NodeExecutor = {
      type: 'shell',
      execute: async (ctx) =>
        ctx.node.nodeKey === 'plan'
          ? { status: 'completed', output: { finalResponse: 'not an array' } }
          : { status: 'completed', output: { stdout: 'x' } },
    };
    const engine = new WorkflowEngine({
      store,
      emit: async () => undefined,
    ...
    expect(node?.error).toContain('did not resolve to an array');
  });
});
